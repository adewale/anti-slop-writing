...
- Ran a two-round parataxis hillclimb (`evals/results/2026-06-13-parataxis-hillclimb/`) with a rate study on the pivotal case. A drafted `Parataxis density` / `Parataxis repair` rule for `SKILL.md` did not clear the gate (round 1 N=6 delta -0.028, CI [-0.25,+0.17]; round 2 discriminating case before 0.833 vs after 0.917, within noise) because the pre-edit doctrine already catches document-level parataxis via the staccato "keep or use once" rule and the symmetrical-structure tell. Reverted in `SKILL.md`; logged in `evals/rejected-edits.md`.
