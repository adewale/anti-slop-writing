...
- Added a new eval kind, the **reference-anchor**: a `holdout` case that pins a fixed, human-reviewable ceiling for one high-value rewrite and uses it as a don't-regress gate for future models and skill versions. First instance: `evals/rewrite-evals.json` → `tweet-best-rewrite-anchor`, with the original and the best known rewrite stored under `evals/fixtures/tweet-taste-as-model/` (`best-rewrite.md`, `HUMAN-REVIEW.md`), the reference scorecard (1.0/1.0) under `evals/results/2026-06-13-tweet-best-anchor/`, the protocol in `docs/reference-anchor-tests.md`, and enforcement in `scripts/validate.py` (referenced artifacts must exist; reference scores must be in [0,1]).
