...
- [ ] **Hand the upstream spec to a harness-repo agent.** `docs/upstream-skill-eval-harness-spec.md` carries five proposals for `adewale/skill-eval-harness` (declarative script-assertion checks, oracle verify hook, migrate stamping `should_trigger`, validate/grade version parity, judge-row quarantine), anchored to harness commit `2297000` and to this repo's incidents. The receiving agent should reconcile against in-progress PRs/issues and report a disposition per proposal; record those dispositions here and close this item.
