...
- **Upstream candidates (proposals only, different repo)**: specified in full in `docs/upstream-skill-eval-harness-spec.md` — five proposals (declarative checks on script assertions, an attestation hook so `oracle: "strong"` is earned by a green self-test, `migrate` stamping derivable `should_trigger` booleans, validate/grade version-strictness parity, and per-row quarantine for malformed judge verdicts), each anchored to harness symbols at the audited commit and to the downstream incident that motivated it. Written to be handed to an agent in the harness repo for reconciliation against its in-progress PRs, issues, and roadmap spec.
