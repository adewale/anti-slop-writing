...
A reference-anchor is an eval case that pins a **fixed, human-reviewable ceiling** for a
...
the skill catches a failure pattern; an anchor measures whether the best achievable rewrite
of one concrete piece still scores at the top.

## When to add one

- You have done deep work on one real artifact (a tweet, a post, a README) and produced a
  rewrite you are willing to call the ceiling.
...
- The `assertions` characterize *why* the best rewrite scores at the top — they are the
...
- `reference_score` / `reference_graded_score` are the scores the stored `best_rewrite`
  achieves against those assertions/dimensions (in [0,1]). Record the scorecard that
  produced them.
- The `best_rewrite` is the ceiling, not a one-shot skill output. It may be hand-tuned by
  an expert/model against the skill's own detectors. It is editable: replace it (and
  re-record the scorecard) only with a strictly better rewrite.
...
3. Compare `candidate_score` to `reference_score`. Track the gap over versions; a drop in
   `candidate_score` across skill versions is a regression.
