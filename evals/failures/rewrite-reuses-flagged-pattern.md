...
`evals/results/2026-05-28-joe-beda-before.md`. The skill was given the
...
For Pattern C, the doctrine wording was ambiguous enough that the
result notes called a correct `revise` verdict a model failure. The
classification rule had to state the both-sides test explicitly.
...
```txt
Verdict: revise
Slop tells: Compressed antithesis. The prior sentence evidences movability ("you own and can move"), but it does not evidence intentionality — that portability is the protocol's design rather than an incidental property.
Concrete rewrite: ATproto stores records in a PDS you own and can move; name the protocol feature that keeps those records stable across a move, or ask the author for the design rationale.
Rewrite check: passes self-detectors — no X-not-Y cadence, no invented design mechanism.
```
