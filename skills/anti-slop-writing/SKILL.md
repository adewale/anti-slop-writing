...
Teaser boundaries: the rule above is forward-pointing only — a back-pointing label (“that last one is the counterintuitive one”) is significance inflation, not a teaser. Discharge is necessary, not sufficient: cut a discharged frame when the payload’s own specifics already carry the emphasis it claimed (“What surprised me most was...”). And a scarcity frame (“what nobody tells you”) also claims the knowledge is rare, which discharging does not repair.
